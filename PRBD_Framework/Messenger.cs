// Credits: https://mvvmfoundation.codeplex.com
...
namespace PRBD_Framework {
    ...
    public class Messenger {
        ...
        public Guid Register(Enum message, Action callback) {
            return this.Register(message, callback, null);
        }
        ...
        /// <param name="message">The message to register for.</param>
        /// <param name="callback">The callback to be called when this message is broadcasted.</param>
        public Guid Register<T>(Enum message, Action<T> callback) {
            return this.Register(message, callback, typeof(T));
        }

        //public void UnRegister(Enum message, Action callback) {
        //    this.UnRegister(message, callback, null);
        //}

        public void UnRegister(Enum message, Guid id) {
            _messageToActionsMap.RemoveAction(message, id);
        }

        //public void UnRegister<T>(Enum message, Action<T> callback) {
        //    this.UnRegister(message, callback, typeof(T));
        //}

        Guid Register(Enum message, Delegate callback, Type parameterType) {
            if (callback == null) {
            ...
            return _messageToActionsMap.AddAction(message, callback.Target, callback.Method, parameterType);
        }

        //void UnRegister(Enum message, Delegate callback, Type parameterType) {
        //    if (callback == null) {
        //        throw new ArgumentNullException("callback");
        //    }

        //    _messageToActionsMap.RemoveAction(message, callback.Target, callback.Method, parameterType);
        //}

        [Conditional("DEBUG")]
        void VerifyParameterType(Enum message, Type parameterType) {
        ...
        public void NotifyColleagues(Enum message, object parameter) {
        ...
        private class MessageToActionsMap {
            ...
            internal Guid AddAction(Enum message, object target, MethodInfo method, Type actionType) {
                if (method == null) {
                ...
                lock (_map) {
                    if (!_map.ContainsKey(message)) {
                        _map[message] = new Dictionary<Guid, WeakAction>();
                    }

                    var id = Guid.NewGuid();
                    _map[message][id] = new WeakAction(target, method, actionType);
                    return id;
                }
            }

            //internal void RemoveAction(Enum message, object target, MethodInfo method, Type actionType) {
            //    if (method == null) {
            //        throw new ArgumentNullException("method");
            //    }
            ...
            //        _map[message].Remove(target as WeakAction);
            //    }
            //}

            internal void RemoveAction(Enum message, Guid id) {
                lock (_map) {
                    _map[message].Remove(id);
                }
            }
            ...
            internal bool TryGetParameterType(Enum message, out Type parameterType) {
                parameterType = null;
                Dictionary<Guid, WeakAction> weakActions;
                lock (_map) {
                    if (!_map.TryGetValue(message, out weakActions) || weakActions.Count == 0) {
                        return false;
                    }
                }
                foreach (var val in weakActions.Values) {
            ...
            readonly Dictionary<Enum, Dictionary<Guid, WeakAction>> _map = new Dictionary<Enum, Dictionary<Guid, WeakAction>>();
